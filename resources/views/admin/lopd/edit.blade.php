<x-layouts.app :title="__('Editar documento LOPD')">
            ...
            <form action="{{ route('admin.lopd.update', $lopd->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class='mb-4'>
                    <flux:select label="Socio" name="socio_id" id="socio_id" required searchable>
                        ...
                        <option value="{{ $socio->id }}" {{ $lopd->socio_id == $socio->id ? 'selected' : '' }}>
                ...
                <div class='mb-4'>
                    <flux:select label="Categoría" name="categoria_id" id="categoria_id" required>
                        ...
                        <option value="{{ $categoria->id }}" {{ $lopd->categoria_id == $categoria->id ? 'selected' : ''
                            }}>
                            {{ $categoria->nombre }}
                        </option>
                        @endforeach
                ...
                <div class='mb-4'>
                    <flux:textarea label="Descripción del documento" name="descripcion" id="descripcion"
                        ...
                        {{ old('descripcion', $lopd->descripcion) }}
                    </flux:textarea>
                </div>
                ...
                <div class='mb-4'>
                    <flux:input label="Fecha de firma" name="fecha_firma" id="fecha_firma" type="date"
                        value="{{ old('fecha_firma', $lopd->fecha_firma ? \Carbon\Carbon::parse($lopd->fecha_firma)->format('Y-m-d') : now()->format('Y-m-d')) }}"
                        required />
                </div>
                ...
                <div class='mb-4'>
                    ...
                    @if($lopd->archivo)
                    <a href="{{ route('lopd.view', basename($lopd->archivo)) }}" target="_blank" rel="noopener"
                        class="text-blue-600 underline mr-2">
                        {{ $lopd->nombre_archivo ?? 'Ver archivo' }}
                    </a>
                    <a href="{{ route('lopd.download', basename($lopd->archivo)) }}" class="text-green-700 underline"
                        download>
                        Descargar
                    </a>
                    @else
                ...
                <div class='mb-4'>
                    <flux:select label="Estado" name="estado_id" id="estado_id" required>
                        ...
                        <option value="{{ $estado->id }}" {{ $lopd->estado_id == $estado->id ? 'selected' : '' }}>
                            {{ $estado->nombre }}
                        </option>
                        @endforeach
                    </flux:select>
                ...
                <div class='mb-4'>
                    <flux:textarea label="Observaciones" name="observaciones" id="observaciones"
                        placeholder="Observaciones adicionales">{{ old('observaciones', $lopd->observaciones) }}
                    </flux:textarea>
                </div>
