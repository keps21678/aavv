        ...
        <table id="tabla" class="display w-full text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    <td class="px-2 py-4">{{ $recibo->id }}</td>
                    <td class="px-2 py-4">{{ $recibo->socio->nombre ?? 'N/A' }}</td>
                    <td class="px-2 py-4">{{ $recibo->cuota->nombre ?? 'N/A' }}</td>
                    <td class="px-2 py-4">{{ number_format($recibo->importe, 2) }} €</td>
                    <td class="px-2 py-4">
                        <span class="px-2 py-1 rounded-full text-sm text-white" style="background-color: {{ $recibo->estado->color }}">
                            {{ $recibo->estado->nombre }}
                        </span>
                    </td>
                    <td class="px-2 py-4">{{ $recibo->fecha_emision->format('Y-m-d') }}</td>
                    <td class="px-2 py-4">
